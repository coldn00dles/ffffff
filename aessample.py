...
def mix_column(column):
    r = [
        secmul(column[0]) ^ thirdmul(
            column[1]) ^ column[2] ^ column[3],
        secmul(column[1]) ^ thirdmul(
            column[2]) ^ column[3] ^ column[0],
        secmul(column[2]) ^ thirdmul(
            column[3]) ^ column[0] ^ column[1],
        secmul(column[3]) ^ thirdmul(
...
def encryptor(key, data):
    ...
    for round in range(1, 10):
        ...
        for grid in grids:
            sub_bytes_step = [[byte_look(val) for val in row] for row in grid]
...
def decryptor(key, data):
    ...
    for grid in grids:
        ...
        sub_bytes_step = [[reverse_byte_look(val) for val in row]
    ...
    for round in range(9, 0, -1):
        ...
        for grid in grids:
            ...
            sub_bytes_step = [
                [reverse_byte_look(val) for val in row] for row in shift_rows_step]
            temp_grids.append(sub_bytes_step)
