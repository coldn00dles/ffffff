...
def hextotext(hex_text):
    text = codecs.decode(hex_text,'hex')
    return text
...
# Receive the data (encrypted text and identifier) from the client
block_received = client_socket.recv(1024).decode()

cipher_text,reversed_round_keys = block_received.split("|")

print(f"Cipher text received : {cipher_text}")

rev_key = [reversed_round_keys[i:i+48] for i in range(0,768,48)]

deciphered_plaintext = hextotext(binarytohex(des_encrypt(cipher_text,rev_key)).lower())

client_socket.send(deciphered_plaintext)
